...
public class AdminService : IAdminService
{
    ...
    public async Task<IEnumerable<UserResponseDto>> GetAllUsersAsync(string? roleFilter, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var user in users)
        {
            ...
            var userDto = await MapToUserResponseAsync(user, roles.ToList());
            result.Add(userDto);
        }
    ...
    public async Task<UserResponseDto> UpdateUserAsync(string userId, UpdateUserRequestDto request, CancellationToken cancellationToken = default)
    {
        var user = await _userManager.FindByIdAsync(userId);
    ...
    public async Task<UserResponseDto> BlockUserAsync(string userId, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new BusinessRuleViolationException("Cannot block an admin user.");
        }

        if (!user.IsActive)
        {
            throw new BusinessRuleViolationException($"User {user.Email} is already blocked.");
        }
    ...
    public async Task<UserResponseDto> UnblockUserAsync(string userId, CancellationToken cancellationToken = default)
    {
        var user = await _userManager.FindByIdAsync(userId);
        ...
        if (user.IsActive)
        {
            throw new BusinessRuleViolationException($"User {user.Email} is already active.");
        }
    ...
    public async Task<UserResponseDto> RemoveAdminAsync(string userId, CancellationToken cancellationToken = default)
    {
        var user = await _userManager.FindByIdAsync(userId);
