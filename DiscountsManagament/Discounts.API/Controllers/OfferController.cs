...
public class OfferController : ControllerBase
{
    ...
    {
        var result = await _offerService.GetOfferDetailsAsync(id, cancellationToken);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> CreateOffer([FromBody] CreateOfferRequestDto request, CancellationToken cancellationToken)
    {
        var userId = User.FindFirstValue(ClaimTypes.NameIdentifier)!;
        var result = await _offerService.CreateOfferAsync(userId, request, cancellationToken);
        return CreatedAtAction(nameof(GetOfferDetails), new { id = result.Id }, result);
    }

    [HttpPut("{id:int}")]
    [Authorize(Roles = Roles.Merchant)]
    public async Task<IActionResult> UpdateOffer(int id, [FromBody] UpdateOfferRequestDto request, CancellationToken cancellationToken)
    {
        var userId = User.FindFirstValue(ClaimTypes.NameIdentifier)!;
    ...
    {
        var userId = User.FindFirstValue(ClaimTypes.NameIdentifier)!;
        var result = await _offerService.GetMerchantOffersAsync(userId, cancellationToken);
        return Ok(result);
    }
    ...
    {
        var userId = User.FindFirstValue(ClaimTypes.NameIdentifier)!;
        var result = await _offerService.GetSalesHistoryAsync(userId, cancellationToken);
        return Ok(result);
    }
    ...
    {
        var result = await _offerService.GetPendingOffersAsync(cancellationToken);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> ApproveOffer(int id, CancellationToken cancellationToken)
    {
        ...
        var result  = await _offerService.ApproveOfferAsync(adminId, id, cancellationToken);
        return Ok(result);
    }

    [HttpPost("{id:int}/reject")]
    [Authorize(Roles = Roles.Admin)]
    public async Task<IActionResult> RejectOffer(int id, [FromBody] RejectOfferRequestDto request, CancellationToken cancellationToken)
    {
        var adminId = User.FindFirstValue(ClaimTypes.NameIdentifier)!;
